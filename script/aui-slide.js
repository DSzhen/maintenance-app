...
(function(window) {
	...
	auiSlide.prototype._init = function(options) {
		...
		this.container.style.height = this.options.height / 3 + "px";
		...
		if(this.slideNodeListLength > 1) {
			...
			this.slideWrap.addEventListener('touchstart', this.touchStartBind, false);
			this.slideWrap.addEventListener('touchmove', this.touchMoveBind, false);
			this.slideWrap.addEventListener('touchend', this.touchEndBind, false);
		}
	};
	// 当分页器为圆点时位置设置
	...
	auiSlide.prototype.destroy = function() {
		if(interval) {
		...
		this.slideWrap.removeEventListener('touchstart', this.touchStartBind, false);
		this.slideWrap.removeEventListener('touchmove', this.touchMoveBind, false);
		this.slideWrap.removeEventListener('touchend', this.touchEndBind, false);
	}
