	...
	auiListSwipe.prototype = {
		...
		toggleEvents:function(element,callback){
			...
			element.addEventListener('touchend', function(event){
				...
				var translateX = $(".aui-list-item-inner").css("transform").replace(/[^0-9\-,]/g,'').split(',')[0].split("-")[1]
				console.log(translateX)
				if(translateX=="undefined" || translateX===undefined){
					$(".wb-dele").css({transition: "400ms", transform: "translateX(0px)"})
				}
			},true)
		},
		...
		deleteReset:function(){
			$(".wb-dele").css({transition: "400ms", transform: "translateX(0px)"})
			if(document.querySelector(".aui-swipe-opened")){
				this.setTranslate(document.querySelector(".aui-swipe-opened"),"0px");
				document.querySelector(".aui-swipe-opened").classList.remove("aui-swipe-opened");
			}
		}
	}
